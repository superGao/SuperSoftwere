...
import com.d3rich.circledimageview.R;
...
public class CircledImageView extends ImageView {
  ...
  public CircledImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    int index = a.getInt(R.styleable.CircledImageView_android_scaleType, -1);
    if (index >= 0) {
    ...
    cornerRadius = a.getDimensionPixelSize(R.styleable.CircledImageView_riv_corner_radius, -1);
    borderWidth = a.getDimensionPixelSize(R.styleable.CircledImageView_riv_border_width, -1);
    ...
    borderColor = a.getColorStateList(R.styleable.CircledImageView_riv_border_color);
    if (borderColor == null) {
      borderColor = ColorStateList.valueOf(CircledDrawable.DEFAULT_BORDER_COLOR);
    }

    mutateBackground = a.getBoolean(R.styleable.CircledImageView_riv_mutate_background, false);
    isOval = a.getBoolean(R.styleable.CircledImageView_riv_oval, false);

    final int tileMode = a.getInt(R.styleable.CircledImageView_riv_tile_mode, TILE_MODE_UNDEFINED);
    if (tileMode != TILE_MODE_UNDEFINED) {
      setTileModeX(parseTileMode(tileMode));
      setTileModeY(parseTileMode(tileMode));
    }

    final int tileModeX = a.getInt(R.styleable.CircledImageView_riv_tile_mode_x, TILE_MODE_UNDEFINED);
    if (tileModeX != TILE_MODE_UNDEFINED) {
      setTileModeX(parseTileMode(tileModeX));
    }

    final int tileModeY = a.getInt(R.styleable.CircledImageView_riv_tile_mode_y, TILE_MODE_UNDEFINED);
  ...
  public void setImageDrawable(Drawable drawable) {
    mResource = 0;
    mDrawable = CircledDrawable.fromDrawable(drawable);
    updateDrawableAttrs();
  ...
  public void setImageBitmap(Bitmap bm) {
    mResource = 0;
    mDrawable = CircledDrawable.fromBitmap(bm);
    updateDrawableAttrs();
    super.setImageDrawable(mDrawable);
  }
  ...
  private Drawable resolveResource() {
    ...
    return CircledDrawable.fromDrawable(d);
  }
  ...
  private void updateBackgroundDrawableAttrs(boolean convert) {
    if (mutateBackground) {
      if (convert) {
        mBackgroundDrawable = CircledDrawable.fromDrawable(mBackgroundDrawable);
      }
  ...
  public void setBorderColor(ColorStateList colors) {
    ...
    borderColor =
        (colors != null) ? colors : ColorStateList.valueOf(CircledDrawable.DEFAULT_BORDER_COLOR);
    updateDrawableAttrs();
    updateBackgroundDrawableAttrs(false);
    if (borderWidth > 0) {
      invalidate();
