package com.supergao.software.view;
...
public class DragLayout extends FrameLayout {
    ...
    protected void onFinishInflate() {
        ...
        vg_main = (MenuLayout) getChildAt(isShowShadow ? 2 : 1);
...
//    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        View bannerView = findViewById(R.id.rl_homepage_banner);
        int[] location = new int[2];
        Rect rect = new Rect();
        bannerView.getLocationInWindow(location);
        bannerView.getHitRect(rect);
        int x = (int) ev.getX();
        int y = (int) ev.getY();
        if (x > location[0] && x < location[0] + rect.right
                && y > location[1] && y < location[1] + rect.bottom) {
            return false;
        }
